...
export const ViewItem = ({ erc, contractAddress, id, amount }) => {
  ...
  return (
    <TokenRow>
      <Wrap
        padding={metadata !== undefined ? "0 0 0 .75rem" : ".4rem 0 0 .75rem"}
        display="flex"
      >
            ...
            {Number(erc) === 721 || Number(erc) === 1155 ? (
              <Text textAlign="left" fontSize="0.9rem">
                <ExternalLink
                  fontSize="0.9rem"
                  color="#aaa"
                  href={`https://ftmscan.com/token/${contractAddress}?a=${Number(
                    id
                  )}`}
                  target="_blank"
                >
