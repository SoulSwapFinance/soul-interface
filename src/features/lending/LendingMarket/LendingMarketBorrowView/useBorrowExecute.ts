...
export const useBorrowExecute: UseBorrowExecute = () => {
  ...
  return useCallback(
    async ({ trade, permit, collateralAmount, borrowAmount, leveraged, spendFromWallet, receiveInWallet }) => {
      ...
      const cooker = new UnderworldCooker(market, account, library, chainId)
      ...
      if (borrowAmount) {
        ...
        if (leveraged && spendFromWallet && collateralAmount) {
          cooker.coffinDepositCollateral(BigNumber.from(collateralAmount.quotient.toString()))
        }
        // Add borrow action
        cooker.borrow(
          BigNumber.from(borrowAmount.quotient.toString()),
          leveraged || !receiveInWallet,
          leveraged ? SOULSWAP_MULTISWAPPER_ADDRESS[chainId] : ''
        )
      }
