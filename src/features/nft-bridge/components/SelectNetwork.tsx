...
import { setLocalRPC } from 'config/nft-bridge'
...
export const ContentWrapper = styled.div`
  width: 100%;
  height: ${config[chainId].isOpenBridge ? '85%' : 'auto'};
  overflow:auto;
  background-color: ${({ theme }) => theme.contentBg};
  ...
  return (
    <>
      ...
      <HideSmall onClick={() => toggleNetworkModal()}>
        {<NetworkCard title={config[chainId].networkName}>
          <TokenLogo symbol={config[chainId].networkLogo ?? config[chainId].symbol} size={'20px'} style={{marginRight:'5px'}}></TokenLogo> 
          {config[chainId].name}
        </NetworkCard>}
