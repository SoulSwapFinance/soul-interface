...
export const useUnderworldBorrowPositions = (pairs: any[]) =>
  useSearchAndSort(
...
export const useUnderworldLendPositions = (pairs: any[]) =>
  useSearchAndSort(
...
export const useLendPositionAmounts = (): PairWithAmount[] => {
  ...
  return useUnderworldLendPositions(pairs)
    .items.map((item) => {
      ...
      return {
        pair: item,
        amount: CurrencyAmount.fromRawAmount(lentAsset, lentAssetAmount).multiply(LEND_MULTIPLIER(chainId, item.address)),
      }
    })
...
export function useUnderworldPositions() {
  ...
  return useMemo(
    ...
    [borrowPositionAmounts, collateralPositionAmounts, underworldBalances, lentPositionAmounts]
  )
}