...
import { FantiesPids, StablePids, CorePids, SoulSwapPids } from '../Pids'
...
function useSoulSummoner(pid, lpToken, token1Address, token2Address) {
  ...
  const soulContract = useTokenContract(SoulSwapPids[0].token1Address[chainId]) // ?
  const fusdContract = useTokenContract(SoulSwapPids[0].token2Address[chainId]) // ?
  ...
  const fusdPerSoul = async () => {
    try {
      const totalSoul = await soulContract.balanceOf(SoulSwapPids[0].lpAddresses[chainId]) // ?
      const totalFusd = await fusdContract.balanceOf(SoulSwapPids[3].lpAddresses[chainId]) // ?
