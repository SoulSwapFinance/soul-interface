...
const FarmRowRender = ({ pid, lpSymbol, lpToken, token1, token2, farm }) => {
  ...
  const getAprAndLiquidity = async () => {
    try {
      const result = await fetchAprAndLiquidity(
        farm.pid,
        farm.token1,
        farm.token2,
        farm.token1Address[4002],
        farm.token2Address[4002],
        lpToken
      );
          ...
          <DetailsContainer>
            <DetailsWrapper>
              ...
              <FunctionBox>
                ...
                <Wrap padding='0' display="flex" justifyContent="space-between">
                  <Text padding='0' color="#f13636">Fee Amount: {ethers.utils.formatUnits(feeAmount)}</Text>
                  <Text padding='0' color="#f13636">Receiving: {ethers.utils.formatUnits(receiving)}</Text>
                </Wrap>
              </FunctionBox>
            </DetailsWrapper>
          </DetailsContainer>
