...
import { Box, BoxProps } from 'features/nft/components/Box'
import { Column, Row } from 'features/nft/components/Flex'
import { body, bodySmall, headlineMedium, headlineSmall } from 'features/nft/css/common.css'
...
import { GenieCollection, TokenType } from 'features/nft/types'
import { floorFormatter, quantityFormatter, roundWholePercentage, volumeFormatter } from 'features/nft/utils/numbers'
    ...
    <>
      <MobileSocialsOverflowIcon onClick={toggleCollectionSocials}>
        {collectionSocialsIsOpen ? (
          ...
          <EllipsisIcon width="28" height="20" 
          // fill={themeVars.colors.textSecondary} 
          />
        )}
        ...
        >
          ...
          {collectionStats.instagram ? (
            <MobileSocialsIcon href={'https://instagram.com/' + collectionStats.instagram}>
              <Box 
              // margin="auto" paddingLeft="2" paddingTop="4"
              >
                <InstagramIcon 
                // fill={themeVars.colors.textSecondary} 
...
const SocialsIcon = ({ children, href }: { children: ReactNode; href: string }) => {
  return (
    <Column as="a" target="_blank" rel="noreferrer" href={href} 
...
export const CollectionStats = ({ stats, isMobile }: { stats: GenieCollection; isMobile: boolean }) => {
  ...
  return (
    <Box
      className="flex relative col w-full justify-center"
    ...
    >
      {isCollectionStatsLoading && (
        ...
        // borderRadius="round" 
        // position="absolute" 
        className={styles.collectionImageIsLoadingBackground} />
      )}
      <Box
