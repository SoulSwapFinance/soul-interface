...
} from 'services/graphql/data/nft/Asset'
import useDebounce from 'hooks/useDebounce'
import { AnimatedBox, Box } from 'features/nft/components/Box'
import { CollectionSearch, FilterButton } from 'features/nft/components/collection'
import { CollectionAsset } from 'features/nft/components/collection/CollectionAsset'
import * as styles from 'features/nft/components/collection/CollectionNfts.css'
import { SortDropdown } from 'features/nft/components/common/SortDropdown'
import { Center, Column, Row } from 'features/nft/components/Flex'
import { SweepIcon } from 'features/nft/components/icons'
import { bodySmall, buttonTextMedium, headlineMedium } from 'features/nft/css/common.css'
...
import { useIsCollectionLoading } from 'features/nft/hooks/useIsCollectionLoading'
import { usePriceRange } from 'features/nft/hooks/usePriceRange'
import { DropDownOption, GenieAsset, GenieCollection, isPooledMarket, Markets, TokenType } from 'features/nft/types'
import {
  calcPoolPrice,
  calcSudoSwapPrice,
  getRarityStatus,
  ...
  pluralize,
} from 'features/nft/utils'
import { scrollToTop } from 'features/nft/utils/scrollToTop'
import { applyFiltersFromURL, syncLocalFiltersWithURL } from 'features/nft/utils/urlParams'
...
export const CollectionNfts = ({ contractAddress, collectionStats, rarityVerified }: CollectionNftsProps) => {
  ...
  const assetQueryParams: AssetFetcherParams = {
    ...
    //   marketplaces: markets.length > 0 ? markets.map((market) => market.toUpperCase() as NftMarketplace) : undefined,
      marketplaces: markets.length > 0 ? markets.map((market) => market.toUpperCase() as any) : undefined,
