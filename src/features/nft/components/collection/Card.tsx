...
import {
  ...
  RarityVerifiedIcon,
  VerifiedIcon,
} from 'features/nft/components/icons'
import { body, bodySmall, buttonTextMedium, subhead } from 'features/nft/css/common.css'
import { themeVars } from 'features/nft/css/sprinkles.css'
import { useIsMobile } from 'features/nft/hooks'
import { GenieAsset, Rarity, TokenType, WalletAsset } from 'features/nft/types'
import { fallbackProvider, isAudio, isVideo, putCommas } from 'features/nft/utils'
import { floorFormatter } from 'features/nft/utils/numbers'
import {
  createContext,
  MouseEvent,
  ReactNode,
...
const baseHref = (asset: GenieAsset | WalletAsset) => {
  if ('address' in asset) return `/#/nft/asset/${asset.address}/${asset.tokenId}?origin=collection`
  if ('asset_contract' in asset) return `/#/nft/asset/${asset.asset_contract.address}/${asset.tokenId}?origin=profile`
  return '/#/nft/profile'
}
