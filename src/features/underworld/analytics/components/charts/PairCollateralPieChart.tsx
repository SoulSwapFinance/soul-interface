...
const PairCollateralPieChart = ({
  ...
  const valueFuncs = {
    supply: (underworldPair: UnderworldPair) =>
      BigNumber.from(underworldPair.totalAsset)
        .add(BigNumber.from(underworldPair.totalBorrow))
        .toNumber() / 100.0,
    asset: (underworldPair: UnderworldPair) =>
      BigNumber.from(underworldPair.totalAsset).toNumber() / 100.0,
    borrow: (underworldPair: UnderworldPair) =>
      BigNumber.from(underworldPair.totalBorrow).toNumber() / 100.0,
  }
