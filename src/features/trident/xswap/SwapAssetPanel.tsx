  ...
  return (
    ...
    p-3 flex flex-col gap-4`}>
      ...
      <div className="flex gap-1 justify-between items-baseline px-1.5">
        ...
        />
        <BalancePanel {...{ disabled, currency, chainId, onChange, spendFromWallet }} />
      </div>
    </div>
  )
}
...
const BalancePanel: FC<Pick<SwapAssetPanel, 'disabled' | 'currency' | 'chainId' | 'onChange' | 'spendFromWallet'>> = ({
  chainId,
  disabled,
  currency,
  onChange,
...
const SwapAssetPanelHeader: FC<
  Pick<
    SwapAssetPanel,
    'currency' | 'currencies' | 'chainId' | 'onSelect' | 'walletToggle' | 'spendFromWallet' | 'disabled' | 'onChange' | 'value'
  > & { label: string; id?: string }
> = ({ walletToggle, currency, chainId, onSelect, spendFromWallet, id, currencies }) => {
  const { i18n } = useLingui()
  // const { chainId } = useActiveWeb3React()
