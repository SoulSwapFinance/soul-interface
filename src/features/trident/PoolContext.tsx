import { ConstantProductPool, Currency, CurrencyAmount, PoolState, Percent, Token, ZERO, Rebase, ChainId, JSBI } from 'sdk'
...
const PoolContext: FC = ({ children }) => {
  ...
  const { currencies, twap, fee } = useCurrenciesFromURL(chainId)
  ...
  const poolBalance = useTokenBalance(chainId, account ?? undefined, poolWithState?.pool?.liquidityToken)
  ...
  return (
    <Context.Provider
      value={useMemo(
        ...
        [currencies, chainId, liquidityValue, noLiquidity, poolBalance, poolShare, poolWithState, rebases, totalSupply]
      )}
    >
