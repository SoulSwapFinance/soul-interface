...
import { useSoulContract } from '../../hooks'
...
export default function TokenStatsModal({ token }: { token: any }) {
  ...
  if (token.symbol == 'FTM') tokenInfo = { circulatingSupply: '2000000', burnt: '0', totalSupply: '0', vaults: '0' }

  const price = formatNumberScale(priceData?.[token.symbol.toLowerCase()], true, 2)

  const modalOpen = useModalOpen(token.symbol == 'SOUL' ? ApplicationModal.SOUL_STATS : ApplicationModal.FTM_STATS)
