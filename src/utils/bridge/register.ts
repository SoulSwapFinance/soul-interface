...
// import {USE_VERSION, timeout} from 'config/constants'
...
export function registerSwap (hash:string, chainId:any) {
  return new Promise(resolve => {
    console.log(hash)
    const url = `${config[chainId].bridgeApi}/v3/register?hash=${hash}&chainId=${chainId}&type=${USE_VERSION}`
...
export function recordsTxns ({
  ...
  return new Promise(async(resolve) => {
    // console.log(hash)
    const url = `${config[chainId].bridgeApi}/v3/records`
...
export function recordsApprove ({
  ...
  return new Promise(async(resolve) => {
    // console.log(hash)
    const url = `${config[chainId].bridgeApi}/v3/records/approved`
    const data = {
      token,
      spender,
      account,
...
export function getP2PInfo (account:any, chainId:any, symbol:string, token:any) {
  return new Promise(resolve => {
    ...
    if (lData) {
      ...
      const url = `${config[chainId].bridgeApi}/v2/register/${account}/${chainId}/${symbol}`
