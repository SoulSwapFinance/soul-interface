      ...
      if (a[b.slug]) {
        ...
        temp.forEach((sub: any[], ind: number) => {
          ...
          const newSub: any[] = sub.map(pool => {
            ...
            const percent = new Percent(swapAmount, totalSwapAmount.toString()).toFixed(0, undefined, Rounding.ROUND_HALF_UP)
            p2.swapPercentage = parseInt(percent)
            p2.total = totalSwapAmount.toString()
            return p2
          })
          if (!existed) {
            const percent = new Percent(swapPool.swapAmount || ZERO, totalSwapAmount.toString()).toFixed(
              0,
              undefined,
...
export function getTradeComposition(
  ...
  const calcSwapPercentage = function (tokenIn: string, amount: string): number | undefined {
    ...
    if (exactTokenIn && trade.inputAmount.greaterThan(0)) { // JSBI.BigInt(0))) {
      const percent = new Percent((amount || 0), trade.inputAmount.quotient).toFixed(0)
