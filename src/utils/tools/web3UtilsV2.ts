...
async function useBatchWeb3 (chainId, list) {
  ...
  let results = ''
  try {
    const results = await getBatchWeb3Result(rpc, list)
  } catch (error) {
    console.log('error')
    ...
    if (
      ...
      if (index < len) {
        ...
        const results = await useBatchWeb3(chainId, list)
      } else {
        useNode[chainId] = {
          rpc: rpcArr[0],
          index: 0
...
async function useWeb3 (chainId, property, name, params) {
  ...
  let results = ''
  try {
    const results = await getWeb3Result(rpc, property, name, params)
  } catch (error) {
    console.log('error')
    console.log(error.toString())
    if (
