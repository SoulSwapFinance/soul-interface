...
import { EXPLORER_URL } from "./explorerLink";
...
export const tokensToTokenBalances = async (tokens, address = "") => {
	console.log(address);
...
export const getExplorerLinkForToken = (token) => {
	...
	return `${EXPLORER_URL[chainId]}token/${token.address}`;
};
export const getExplorerLinkForTokenAndAddress = (token, ethAccount) => {
    const { chainId } = useActiveWeb3React()

	return `${EXPLORER_URL[chainId]}token/${token.address}?a=${ethAccount}`;
};
