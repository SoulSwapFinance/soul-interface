...
import SelectChainIDPanel from 'features/nft-bridge/hooks/selectChainId'
import SelectCurrencyPanel from 'features/nft-bridge/hooks/selectCurrency'
...
export default function CroseNFT () {
  ...
  const fetchNFTList = useCallback(() => {
    getUrlData(`${config[chainId].bridgeApi}/v3/nft?chainId=${chainId}&version=NFTV1`).then((res:any) => {
      console.log(res)
  ...
  const tokenList = useMemo(() => {
    if (nftList) {
      ...
      const urlParams = (selectCurrency && selectCurrency?.chainId?.toString() === chainId?.toString() ? selectCurrency?.address : (initBridgeToken ? initBridgeToken : config[chainId].getCurChainInfo(chainId).nftInitToken))?.toLowerCase()
  ...
  function setMetamaskNetwork (item:any) {
    selectNetwork(item).then((res:any) => {
      if (res.msg === 'Error') {
        alert(t('changeMetamaskNetwork', {label: config[chainId].networkName}))
      }
    })
  }
      ...
      <AppBody>
        ...
        <FlexWrapBox>
          ...
          }
          {
            fee ? (
              <FeeBox>
                {t('fee')}: {fromWei(fee, 18)} {config[chainId].symbol}
              </FeeBox>
            ) : ''
          }
        </FlexWrapBox>
