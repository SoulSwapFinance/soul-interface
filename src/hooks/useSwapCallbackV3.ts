...
import { useUserSlippageTolerance, useUserSlippageToleranceCrosschain } from 'state/user/hooks'
...
const useSwapCallbackV3 = (isPermitSwap?: boolean) => {
  ...
  const getSwapData = useCallback(() => {
    ...
    const inputAddress = inputAmount.currency.isNative ? ZERO_ADDRESS : inputAmount.currency.wrapped.address
    const outputAddress = outputAmount.currency.isNative ? ZERO_ADDRESS : outputAmount.currency.wrapped.address
