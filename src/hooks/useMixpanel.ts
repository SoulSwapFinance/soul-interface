...
// const allowedSlippage = useUserSlippageTolerance()
  ...
  const mixpanelHandler = useCallback(
    ...
    [currencies, network, saveGas, account, trade, mixpanel.hasOwnProperty('get_distinct_id'), ethPrice],
  )
  const subgraphMixpanelHandler = useCallback(
    async (transaction: TransactionDetails) => {
      const apolloProMMClient = NETWORKS_INFO[(chainId as ChainId) || (ChainId.ETHEREUM as ChainId)].elasticClient
