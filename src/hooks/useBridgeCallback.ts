...
// import {tryParseAmount1} from 'state/swap/hooks'
import { useTransactionAdder } from '../state/transactions/hooks'
import { useCurrencyBalance, useETHBalances } from '../state/wallet/hooks'
import { useActiveWeb3React } from './index'
import { useSwapUnderlyingContract } from './useContract'
 ...
 */
 export function useCrossBridgeCallback(
  ...
  // console.log(inputCurrency)
  // 我们总是可以解析输入货币的金额，因为包装是1:1
  const inputAmount = useMemo(() => inputCurrency ? tryParseAmount(typedValue, inputCurrency)
   : tryParseAmount(typedValue), [inputCurrency, typedValue])
  //  : tryParseAmount1(typedValue, 18), [inputCurrency, typedValue])
  const addTransaction = useTransactionAdder()
  return useMemo(() => {
...
// ): { execute?: undefined | (() => Promise<void>); inputError?: string } {
  ...
  const sendTx = useCallback(() => {
    ...
    if (!connectedWallet || !account || !inputAmount || ConnectType.EXTENSION !== connectedWallet.connectType) return
