...
const generateCoingeckoUrl = (
  ...
  return `${COINGECKO_API_URL}/coins/${
    NETWORKS_INFO[chainId || ChainId.ETHEREUM].coingeckoNetworkId
...
const fetchSoulDataSWRWithHeader = async (url: string) => {
...
export default function useBasicChartData(tokens: (Token | null | undefined)[], timeFrame: LiveDataTimeframeEnum) {
  ...
  const tokenAddresses = useMemo(
    () =>
      tokens
        .filter(Boolean)
        .map(token => (token?.isNative ? WNATIVE[chainId || ChainId.ETHEREUM].address : token?.address)?.toLowerCase()),
    [tokens, chainId],
  )
