...
import coffinBoxExports from 'constants/abis/soulswap/coffinbox.json'
...
export const getCoffinBoxContractConfig = (chainId: number | undefined) => ({
  addressOrName:
   coffinBoxExports[chainId?.toString() as keyof Omit<typeof coffinBoxExports, '31337'>]?.[0]?.contracts?.BentoBoxV1
      ?.address ?? '',
  contractInterface:
   coffinBoxExports[chainId?.toString() as keyof Omit<typeof coffinBoxExports, '31337'>]?.[0]?.contracts?.BentoBoxV1
      ?.abi ?? [],
