...
import { useUserSlippageToleranceCrosschain } from 'state/user/hooks' // useIsWhiteListKyberAI, useUserSlippageTolerance, 
...
export default function useMixpanelV2(currencies?: { [field in Field]?: Currency }) {
  const { chainId, account } = useActiveWeb3React()
  const { saveGas } = useSwapState()
  const network = NETWORKS_INFO[chainId].name
  ...
  const inputSymbol = inputCurrency && inputCurrency.isNative ? NETWORKS_INFO[chainId].nativeToken.symbol : inputCurrency?.symbol
  const outputSymbol =
    outputCurrency && outputCurrency.isNative ? NETWORKS_INFO[chainId].nativeToken.symbol : outputCurrency?.symbol
  const ethPrice = useETHPrice()
  const dispatch = useDispatch<AppDispatch>()
  // const selectedCampaign = useSelector((state: AppState) => state.campaigns.selectedCampaign)
  const [allowedSlippage] = useUserSlippageToleranceCrosschain()
    ...
    (type: MIXPANEL_TYPE, payload?: any) => {
      ...
      switch (type) {
        ...
        case MIXPANEL_TYPE.SWAP_COMPLETED: {
          ...
          const formattedGas = gas_price ? formatUnits(gas_price, NETWORKS_INFO[chainId].nativeToken.decimal) : '0'
  ...
  const subgraphMixpanelHandler = useCallback(
    ...
    [chainId, dispatch, mixpanelHandler, classicClient, elasticClient], // isEVM
  )
...
export const useGlobalMixpanelEvents = () => {
  ...
  useEffect(() => {
    if (pathName) {
      ...
      protectedPageName && mixpanelHandler(MIXPANEL_TYPE.PAGE_VIEWED, { page: protectedPageName }) // isWhiteList
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
