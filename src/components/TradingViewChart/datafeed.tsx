...
export const checkPairHasDextoolsData = async (
  ...
  const checkedPairs: { [key: string]: { ver: number; pairAddress: string; time: number } } = cPstr
    ? JSON.parse(cPstr)
    : {}
  const symbolA = currencyA.isNative ? NETWORKS_INFO[chainId || ChainId.ETHEREUM].nativeToken.name : currencyA.symbol
  const symbolB = currencyB.isNative ? NETWORKS_INFO[chainId || ChainId.ETHEREUM].nativeToken.name : currencyB.symbol
      ...
      resolveSymbol: async (
        ...
        try {
          const label1 = currencies[0]?.isNative
            ? NETWORKS_INFO[chainId || ChainId.ETHEREUM].nativeToken.name
            : currencies[0]?.symbol
