import { ChainId, Currency, NATIVE, NativeCurrency, Token, WNATIVE_ADDRESS } from 'sdk'
...
export default forwardRef<PairSuggestionHandle, Props>(function PairSuggestionInput(
  ...
  const findToken = (search: string): NativeCurrency | Token | undefined => {
    if (search.toLowerCase() === WNATIVE_ADDRESS[chainId].toLowerCase()) {
      return NATIVE[chainId] // (chainId as ChainId)
    }
  ...
  const onSelectPair = (item: SuggestionPairData) => {
    ...
    if (!isActivePair(activeTokens, item, chainId)) {
      // show import modal
      const newQs = {
        ...qs,
  ...
  return (
    <Wrapper>
      ...
      <MobileView>
        <Modal 
          isOpen={isShowListPair} 
          onDismiss={hideListView} 
          // enableInitialFocusInput={true}
        >
          <WrapperPopup>
