import { ChainId, Currency, Token, WETH } from 'sdk'
...
import { useImportTokenMutation } from 'services/kyberswap/ksSetting'
...
import { filterTokens, filterTruthy } from 'utils/filtering'
...
export function CurrencySearch({
  ...
  const tokenImportsFiltered = useMemo(() => {
    return (debouncedQuery ? filterTokens(tokenImports, chainId, debouncedQuery) : tokenImports).sort(tokenComparator)
  ...
  const filteredCommonTokens = useMemo(() => {
    return filterTokens(commonTokens as Token[], chainId, debouncedQuery).filter(filterWrapFunc)
          ...
          if (rawToken) {
            tokens.push(
              new WrappedTokenInfo({
                ...
                logoURI: null, // rawToken.logoURI || '',
                isWhitelisted: null, // rawToken.isWhitelisted || false,
                multichainInfo: null, // rawToken.multichainInfo || [],
                domainSeparator: null, // rawToken.domainSeparator || '', 
              }),
            )
  ...
  const removeImportedToken = useCallback(
    ...
    [chainId, removeToken], // toggleFavoriteToken, favoriteTokens
  )
    ...
    <ContentWrapper>
      <PaddedColumn gap="14px">
              ...
              text={
                tooltip || (
                  <Text>
                      ...
                      <br />
                      {`You can select and trade any token on SoulSwap.`}
                  </Text>
                )
              }
            />
        ...
        <Text style={{ color: theme.subText, fontSize: 12 }}>
            {`You can search and select ${<span style={{ color: theme.text }}>any token</span>} on SoulSwap`}
        </Text>
      ...
      {isImportedTab && visibleCurrencies.length > 0 && (
        ...
        >
          <div>
            {visibleCurrencies.length} Custom Tokens
          </div>
