import { ChainId, Currency, CurrencyAmount, JSBI, Token, TokenAmount } from 'sdk'
...
function balanceComparator(
  balanceA?: TokenAmount<Token> | CurrencyAmount<Currency>,
  balanceB?: TokenAmount<Token> | CurrencyAmount<Currency>,
) {
...
function getTokenComparator(
  balances: {
    [tokenAddress: string]: TokenAmount<Token> | undefined
  },
...
export function useTokenComparator(inverted: boolean, customChain?: ChainId): (tokenA: Token, tokenB: Token) => number {
  ...
  const ethBalance = useCurrencyBalance(chainId)
  return useMemo(() => {
