...
import { ContentWrapper, NoResult } from 'components/SearchModalMultichain/CurrencySearch'
import CurrencyListBridge from 'components/SearchModalMultichain/bridge/CurrencyListBridge'
import { useTokenComparator } from 'components/SearchModalMultichain/sorting'
import { PaddedColumn, SearchIcon, SearchInput, SearchWrapper, Separator } from 'components/SearchModalMultichain/styleds'
...
export default function CurrencySearchBridge({
  ...
  const filteredTokens: WrappedTokenInfo[] = useMemo(() => {
    ...
    return chainId ? filterTokens(fetchedTokens, chainId, debouncedQuery) : []
          ...
          <Text fontWeight={500} fontSize={20} display="flex">
            ...
            {!isOutput && (
              <InfoHelper
                ...
                size={16}
                text={
                  `You can select and transfer any token supported by the bridge from one chain to another`
                }
              />
            )}
          </Text>
