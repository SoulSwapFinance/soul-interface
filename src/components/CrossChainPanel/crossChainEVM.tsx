...
import LiquidityPool from 'features/bridge/components/LiquidityPool'
...
import { ArrowWrapper, BottomGrouping } from 'features/swap/styleds'
...
export default function CrossChain({
  ...
  const formatInputBridgeValue = tryParseAmount(inputBridgeValue, formatCurrency[chainId] ?? undefined)
//   const formatInputBridgeValue = tryParseAmount(inputBridgeValue, formatCurrency ?? undefined)
  ...
  return (
    <>
      ...
      >
        <LogoBox>
          <CurrencyLogo currency={selectCurrency?.symbol ?? selectCurrency?.symbol} size={'1rem'} />
        </LogoBox>
        <ConfirmContent>
            ...
            account && destConfig?.type !== 'swapin' && !isNaN(selectChain) ? (
              <ArrowWrapper clickable={false} style={{cursor:'pointer', position: 'absolute', right: 0}} onClick={() => {
                  ...
                  {/* BLACK COLOR (TODO) */}
                {
                  swapType === 'swap' ? (
                    <FlexEC>
                      <Plus size="16" color={'#000000'} /> <span style={{fontSize: '12px', lineHeight:'12px'}}>{t('sendto')}</span>
                    </FlexEC>
                  ) : (
                    <FlexEC>
                      <Minus size="16" color={'#000000'} /> <span style={{fontSize: '12px', lineHeight:'12px'}}>{t('sendto')}</span>
                    </FlexEC>
                  )
                }
              </ArrowWrapper>
