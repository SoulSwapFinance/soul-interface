...
import { WrappedTokenInfo } from 'state/lists/bridge'
...
import { CROSS_BRIDGE_LIST } from 'constants/bridges'

function currencyKey(currency: Currency): string {
  return currency instanceof Token ? currency.address : currency?.isNative ? 'ETH' : ''
}
  ...
  const htmlNodes = useMemo(() => {
    ...
    for (const obj of itemData) {
      const isNativeToken = 
        ...
        && obj?.address === config.getCurChainInfo(chainId)?.nativeToken?.toLowerCase()
      ...
      <ListBox style={{height: height}}>
        {
          htmlNodes.map((item, index) => {
            ...
            const isSelected = Boolean(selectedCurrency?.symbol === currency?.symbol )
