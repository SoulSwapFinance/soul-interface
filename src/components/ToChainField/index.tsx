...
interface ToChainField {
  toChain?: ChainId
  action: any
}

const ToChainField: FC<ToChainField> = ({ toChain, action }) => {
  const { i18n } = useLingui()
  const { chainId } = useActiveWeb3React()
  const { toChain, loading } = useState(chainId)
  const dispatch = useAppDispatch()
  const [use, setUse] = useState(false)
  const error = useMemo(
     () => Boolean(!toChain && !loading),
    [toChain, loading]
  )

  // Unset toChain on unmount
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    if (!error && toChain) 
    dispatch(action(toChain))
  }, [action, toChain, dispatch, error])
  ...
  return !use ? (
    <div className="flex justify-center">
      <Button size="xs" 
        color={getChainByColorCode(chainId)} 
        variant="filled" 
