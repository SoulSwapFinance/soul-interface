...
import CurrencySearchModal from 'components/SearchModalMultichain/CurrencySearchModal'
import CurrencySearchModalBridge from 'components/SearchModalMultichain/bridge/CurrencySearchModalBridge'
...
import SelectNetwork from 'features/crosschain/SelectNetwork'
import { EMPTY_ARRAY, ETHER_ADDRESS } from 'constants/index'
...
const useGetPriceUsd = (currency: Currency | undefined, value: string) => {
  ...
  const currencyValueString =
    Number.isNaN(currencyValueInUSD) || Number(value) === 0 ? '' : formatNumber(String(currencyValueInUSD), true)
...
export default function CurrencyInputPanelBridge({
  ...
  const selectedCurrencyBalance = useCurrencyBalance(chainId, account, isOutput ? undefined : currency ?? undefined)
  ...
  const visibleUsdBalance = hasUsdValue ? (usdValue ? formatNumber(usdValue, true) : undefined) : currencyValueString
  useEffect(() => {
            ...
            >
              <Aligner>
                <RowFixed>
                  ...
                  >
                    {currency?.symbol || (loadingToken ? `Loading Tokens` : `Select Token`)}
                  </StyledTokenName>
                </RowFixed>
