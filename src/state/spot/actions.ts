...
import { getKnownTokens } from "utils/markets/knownTokens";
import { 
    USE_ORDERBOOK_PRICES, 
    USE_RELAYER_MARKET_UPDATES } from "constants/index"
import {
...
import {
	getAvailableMarkets,
	marketToString,
} from "features/explorer/Markets";
import { getAllMarketsStatsFromRelayer, getMarketStatsFromRelayer, getRelayer } from "features/explorer/Relayer";
...
export const fetchMarkets = () => {
	return async (dispatch, getState) => {
		...
		if (!USE_ORDERBOOK_PRICES) {
			...
			dispatch(setMarketsStats());
			// dispatch(setMarketsStatsState(ServerState.Done));
			dispatch(setMarketsStatsState());
			const state = getState();
...
export const updateERC20Markets = () => {
    let USE_RELAYER_MARKET_UPDATES = true
	return async (dispatch) => {
		try {
			// tslint:disable-next-line:no-floating-promises
